...
def singleton(cls):
    """
    Function to get single instance of the geodude class
    """
    instances = {}

    def getinstance(**kwargs):
        """
        Creates a new geodude instance if not created already
        """
...
class geodude:
    ...
    def query(self, coordinates: list[tuple[float, float]]) -> Iterable[GeocoderResultBaseModel]:
        ...
        returns:
        geodude Iterator of reverse geocoded locations of type GeocoderResultBaseModel,
        """
        if self.mode == 1:
